...
function getNextFriday(date) {
  const daysToFriday =
    date.getDay() < 5 ? 5 - date.getDay() : 7 - (date.getDay() - 5);
...
function isDateInPeriod(date, period) {
  ...
  return (
    inputDate >= new Date(period.start) && new Date(period.end) >= inputDate
  );
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  while (inputDate <= endDate) {
    ...
    inputDate = new Date(inputDate.setDate(inputDate.getDate() + 1));
  }
...
function getWeekNumberByDate(date) {
  ...
  const firstWeekDaysNum =
    startYearDate.getDay() === 0 ? 1 : 7 - (startYearDate.getDay() - 1);
  const daysFromStartDate = (date - startYearDate) / (1000 * 3600 * 24);
  const startingWeekNumeration = 1;
  const firstWeekNotIncludedAddition = 1;
  return daysFromStartDate < firstWeekDaysNum
    ? 1
    : Math.floor((daysFromStartDate - firstWeekDaysNum) / 7) +
        startingWeekNumeration +
        firstWeekNotIncludedAddition;
}
...
function getNextFridayThe13th(date) {
  let nextThirteenthDay;
  let monthDiff = 0;
  do {
    nextThirteenthDay = new Date(
      date.getFullYear(),
      date.getMonth() + monthDiff,
      13
    );
    monthDiff += 1;
  } while (nextThirteenthDay.getDay() !== 5);
  return nextThirteenthDay;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const [curDateNum, curMonth, curYear] = period.start.split('-').map(Number);
  let currentDate = new Date(curYear, curMonth - 1, curDateNum);
  const [endDateNum, endMonth, endYear] = period.end.split('-').map(Number);
  const endDate = new Date(endYear, endMonth - 1, endDateNum);
  let currentDayIdx = 0;
  const schedule = [];
  while (currentDate <= endDate) {
    if (currentDayIdx % (countWorkDays + countOffDays) < countWorkDays) {
      const workYear = currentDate.getFullYear();
      const workMonth = `0${(currentDate.getMonth() + 1).toString()}`.slice(-2);
      const workDay = `0${currentDate.getDate().toString()}`.slice(-2);
      schedule.push(`${workDay}-${workMonth}-${workYear}`);
    }
